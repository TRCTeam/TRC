...
CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // 2^12 minimum difficulty
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    if(pindexBest->nHeight <= nLastPowBlock) {
        nSubsidy = 25396825 * COIN;
		int factor = (pindexBest->nHeight / 500) + 1;
		nSubsidy /= factor;
    }else{
        return nFees;
    }
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            if (txPrev.nTime > nTime)
                return DoS(100, error("ConnectInputs() : transaction timestamp %u seconds earlier than input transaction",
                                      txPrev.nTime - nTime));
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (GetBlockTime() > FutureDrift((int64_t)vtx[0].nTime))
        return DoS(50, error("CheckBlock() : coinbase timestamp: %u is too early (block timestamp: %"PRIu64")", vtx[0].nTime, GetBlockTime()));
    ...
    BOOST_FOREACH(const CTransaction& tx, vtx)
    {
        ...
        if (GetBlockTime() < (int64_t)tx.nTime)
            return DoS(50, error("CheckBlock() : block timestamp %"PRIu64" seconds earlier than transaction timestamp",
                                 (int64_t)tx.nTime - GetBlockTime()));
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "TorrentCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Welcome to Torrent Coin, a new breakthrough in blockchain storage technology!";
        CTransaction txNew;
        txNew.nTime = 1428226450;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 6;
        block.nTime    = 1428226450;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = fTestNet ? 0 : 1939266;
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x06f26636da6b346e3e226994be7260f9e07e6dc79736840c38b26e456c374c17"));
        block.print();
        ...
        {
            LOCK(mempool.cs);
            mempool.addUnchecked(txhash, txNew);
        }
        
        uint256 genesisHash = fTestNet ? hashGenesisBlockTestNet : hashGenesisBlock;
        
        {
            ...
            CDiskTxPos posThisTx(pindex->nFile, pindex->nBlockPos, nTxPos);
            txdb.UpdateTxIndex(txhash, CTxIndex(posThisTx, txNew.vout.size()));
            txdb.TxnCommit();
        }
    }
...
unsigned char pchMessageStart[4] = { 0xfe, 0xfd, 0xfa, 0xed };
